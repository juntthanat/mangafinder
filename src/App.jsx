...
function App() {
  ...
  const getManga = async () => {
    return await fetch(`${baseUrl}/manga?title=${encodeURIComponent(searchTitleInput)}`).then(e => e.json());
  }
  const getCoverImageFileName = async (coverImageId) => {
    return await fetch(`${baseUrl}/cover/${coverImageId}`).then(e => e.json())
  }
  ...
  const getMangaCoverImage = async () => {
    ...
    const coverImageId = res.data[0].relationships.find(relationship=>relationship.type==="cover_art").id
    const coverImageFileName = (await getCoverImageFileName(coverImageId)).data.attributes.fileName
    setCoverImage((await getCoverImage(mangaId, coverImageFileName)).url)
  }
  const getMangaTitle = async () => {
    setMangaTitle((await fetch(`${baseUrl}/manga?title=${encodeURIComponent(searchTitleInput)}`).then(e => e.json())).data[0].attributes.title.en)
  }
  ...
  const getMangaList = async () => {
    ...
    for(let resIndex = 0; resIndex < 3; resIndex++){
      mangaCoverList.push((await getCoverImage(res.data[resIndex].id, (await getCoverImageFileName(res.data[resIndex].relationships.find(relationship=>relationship.type==="cover_art").id)).data.attributes.fileName)).url)
    }

    setShowMangeList(<div>
    ...
    console.log(mangaCoverList)
  }

  const testHTMLInput = () =>{
    setAddingHTML(<div>hello world</div>)
  }

  // const createListOfManga = () => {

  // }

  return (
    <>
      <input type="text" placeholder="Search Manga" onKeyDown={handleKeyDown}></input>
      <div>
